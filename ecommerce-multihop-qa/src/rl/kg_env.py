...
class Action:
    ...
    # Pre-computed action embedding for the policy network
...
class KGEnvironment:
    ...
    def _resolve_anchor(self, query_repr: QueryRepresentation) -> str:
        """
        Find the best anchor node in Neo4j via semantic similarity search.
        Searches Product nodes by title similarity.
        """
        product_type_hints = query_repr.product_type_hints
        keyword = product_type_hints[0] if product_type_hints else ""
        candidates = self.connector.search_products(
            keyword=keyword,
            limit=20,
        )
        if not candidates:
            raise ValueError("No anchor products found for query.")
        ...
        for cand in candidates:
            node_repr = self.node_enc.encode_node(
                node_type="Product",
                properties=cand,
                node_id=cand["parent_asin"],
            )
            score = self.scorer.score(
                query_repr.embedding,
                node_repr.embedding,
            )
            if score > best_score:
                best_score = score
                best_id = cand["parent_asin"]
