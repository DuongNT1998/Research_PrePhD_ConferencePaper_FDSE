...
class PPOTrainer:
    ...
    def __init__(
        ...
        # query text → gold parent_asins, so rollouts can score answers exactly
        self.gold_map: Dict[str, List[str]] = gold_map or {}
    ...
    def _run_episode(
        ...
        obs, info = self.env.reset(
            query, anchor_id, gold_answers=self.gold_map.get(query)
        )
    ...
    def _ppo_update(self) -> Dict[str, float]:
        ...
        for epoch in range(ppo.ppo_epochs):
            for batch in self.buffer.get_mini_batches(ppo.mini_batch_size):
                ...
                # Defensive guard: never backprop a non-finite loss (a single
                # degenerate batch must not poison the whole network with NaNs).
                if not torch.isfinite(loss):
                    logger.warning(
                        "Non-finite loss (ploss=%.4f vloss=%.4f) — skipping batch.",
                        policy_loss.item() if torch.isfinite(policy_loss) else float("nan"),
                        value_loss.item() if torch.isfinite(value_loss) else float("nan"),
                    )
